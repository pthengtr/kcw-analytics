...
| **SYP** | [`worker_tasks/run_syp_icmas_sync.bat`](worker_tasks/run_syp_icmas_sync.bat) | ICMAS only → Drive + Supabase `raw_syp_icmas_products` |
...
| **HQ** | [`worker_tasks/run_hq_icmas_sync.bat`](worker_tasks/run_hq_icmas_sync.bat) | ICMAS only → Drive + Supabase `raw_hq_icmas_products` |
...
| **HQ** | [`worker_tasks/run_bank_statement_import.bat`](worker_tasks/run_bank_statement_import.bat) | Daily bank sync: BRDET/BPDET + Drive `01_raw/statement` (KBANK+KTB) → `bank.statement_*`, then monthly Excel report |
...
Notebooks resolve Drive via `src.kcw.notebook.setup()` / `KCW_DRIVE_ROOT` (Windows `G:\Shared drives`, Colab Shareddrives, or Linux `~/mnt/gdrive`). Do not hardcode `G:`.
...
python -m src.kcw.pipeline upload-icmas --site hq
python -m src.kcw.pipeline upload-icmas
